...
import { IModelGenerator } from "../interfaces/IModelGenerator.js"


export class ModelGenerator implements IModelGenerator {
  generate(cls: LocalEntity, is_supertype: boolean, relations: RelationInfo[], package_name: string, importedEntities: Map<ImportedEntity, ModuleImport | undefined>) : Generated {
  ...
  return expandToStringWithNL`
    ...
    ${supertype ? this.generateImportSuperEntity(package_name, cls, supertype, importedEntities) : undefined}
    ...
    public ${is_abstract? `abstract` : undefined} class ${cls.name} ${supertype ? `extends ${supertype.name}` : ''} implements Serializable {
      ...
      ${cls.attributes.map(a => this.generateAttribute(a,is_abstract)).join('\n')}
      ${this.generateRelations(cls, relations)}
  ...
  private generateImportSuperEntity (package_name: string, entity: Entity, supertype: Entity, importedEntities: Map<ImportedEntity, ModuleImport | undefined>):string {

    if (isLocalEntity(supertype)){
      return `import ${package_name.replace(entity.$container.name.toLowerCase(),this.generateImportEntity(supertype,importedEntities))}.models.${supertype.name};`
    }
    return `import br.nemo.immigrant.ontology.entity.${this.generateImportEntity(supertype,importedEntities)}.models.${supertype.name};` 

  } 

  private generateImportEntity (entity: Entity, importedEntities: Map<ImportedEntity, ModuleImport | undefined>): string {
  ...
  private generateAttribute(attribute:Attribute, is_abstract:Boolean): Generated{
    return expandToStringWithNL`
    ${this.generateUniqueCollumn(attribute)}
    ${is_abstract? `protected`: `private`} ${capitalizeString(toString(this.generateTypeAttribute(attribute)) ?? 'NOTYPE')} ${attribute.name};
    `
  }

  private generateUniqueCollumn(attribute: Attribute): Generated{
    if (attribute?.unique){
  ...
  private generateTypeAttribute(attribute:Attribute): Generated{
  ...
  private generateRelations(cls: LocalEntity, relations: RelationInfo[]) : Generated {
    
    const node = new CompositeGeneratorNode()

    for(const rel of relations) {
      node.append(this.generateRelation(cls, rel))
      node.appendNewLine()
    }
    return node
  }

  private generateRelation(cls: LocalEntity, {tgt, card, owner}: RelationInfo) : Generated {
    switch(card) {
    case "OneToOne":
      if(owner) {
        return expandToStringWithNL`
  ...
  private createEnum(enumEntityAtribute: EnumEntityAtribute):string {
  ...
  private generateEnum (cls: LocalEntity):string {
    return expandToStringWithNL`
    ${cls.enumentityatributes.map(enumEntityAtribute =>this.createEnum(enumEntityAtribute)).join("\n")}
    `
  }
}