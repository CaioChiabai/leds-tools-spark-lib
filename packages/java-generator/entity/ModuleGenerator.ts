...
export class ModuleGenerator implements IJavaGenerator{
  ...
  const imported_entities = this.processImportedEntities(model)
  ...
  for(const mod of modules) {
    ...
    const supertype_classes = this.processSupertypes(mod)
    ...
    for(const cls of mod_classes) {
      ...
      const {attributes, relations} = this.getAttrsAndRelations(cls, relation_maps)
      
      attributes

      fs.writeFileSync(path.join(MODELS_PATH,`${class_name}.java`), toString(modelGenerator.generate(cls, supertype_classes.has(cls), relations, package_name, imported_entities)))
      if (!cls.is_abstract){
        fs.writeFileSync(path.join(REPOSITORIES_PATH, `${class_name}Repository.java`), toString(this.generateClassRepository(cls, package_name, imported_entities)))
      }
    ...
    for (const enumx of mod.elements.filter(isEnumX)){
      fs.writeFileSync(path.join(MODELS_PATH,`${enumx.name}.java`), enumGenerator.generate(enumx,package_name))
    }
  }
}

  private processImportedEntities (application: Model): Map<ImportedEntity, ModuleImport | undefined> {
   ...
   */
  private processSupertypes(mod: Module) : Set<LocalEntity | undefined> {
   ...
   */
  private getAttrsAndRelations(cls: LocalEntity, relation_map: Map<LocalEntity, RelationInfo[]>) : {attributes: Attribute[], relations: RelationInfo[]} {
    // Se tem superclasse, puxa os atributos e relações da superclasse
    if(cls.superType?.ref != null && isLocalEntity(cls.superType?.ref)) {
      const parent =  cls.superType?.ref
      const {attributes, relations} = this.getAttrsAndRelations(parent, relation_map)
  ...
  private generateClassRepository(cls: LocalEntity, package_name: string, importedEntities: Map<ImportedEntity, ModuleImport | undefined>) : Generated {
