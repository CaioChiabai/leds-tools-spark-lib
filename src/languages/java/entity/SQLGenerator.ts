...
export class SQLGenerator implements IJavaGenerator{
  generate(model: Model, target_folder: string) {

    if (model.configuration){
        // criando a pasta que salva o SQL
        const SQL_PATH = createPath(target_folder, "sql")
        fs.writeFileSync(path.join(SQL_PATH, 'sql_unique_constrains.sql'), toString(this.generateSQL(model)))
        
    }
  }

  private generateSQLCommand (entity: LocalEntity ): Generated {
  ...
  private generateSQL(model:Model): Generated{
      return expandToStringWithNL`
      ${model.abstractElements.filter(isModule).map(module => module.elements.filter(isLocalEntity).map(entity => this.generateSQLCommand(entity)).join('\n')).join('\n')}  
      `
  } 
}
