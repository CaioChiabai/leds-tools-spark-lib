...
export class DocumentationGenerator implements IJavaGenerator{
    generate(model: Model, target_folder: string) : void {
        fs.mkdirSync(target_folder, {recursive:true})
    
        if (model.configuration){
            fs.writeFileSync(path.join(target_folder, 'README.md'),this.createProjectReadme(model.configuration))
            fs.writeFileSync(path.join(target_folder, '.gitlab-ci.yml'),this.createGitlab())
        } 
   
  
    }

    private createGitlab():string{
    ...
    private createProjectReadme(configuration: Configuration): string{
        return expandToStringWithNL`
        # ${configuration.name}
        ## 🚀 Goal
