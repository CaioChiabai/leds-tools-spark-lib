...
function slicer2(cls: LocalEntity, relations: RelationInfo[]): string {
    // Processa os atributos da classe
    const attributes = cls.attributes.map(a => {
        const type = generateTypeAttribute(a); // Gera o tipo correto
        const name = capitalizeString(a.name); // Capitaliza o nome
        return `${type} ${name}`; // Retorna no formato "tipo nome"
    });

    // Processa os enums
    const enums = generateEnum(cls).split(', ').filter(e => e.trim() !== '');

    // Processa as relações
    const relationsProcessed = generateRelationsRequest(cls, relations)
        .split('\n') // Divide as relações em linhas
        .filter(line => line.trim() !== ''); // Remove linhas vazias

    // Junta atributos, enums e relações em uma única linha, separados por vírgulas
    return [...attributes, ...enums, ...relationsProcessed].join(', ');
}
...
namespace ${model.configuration?.name}.Application.UseCase.Entities.${cls.name}Case.Create
{
    /*
     ...
     @entity ${cls.name}
     @operation create
     @param ${slicer2(cls, relations).slice(0, slicer2(cls, relations).lastIndexOf(','))}
     @return ApiResponse
     */
